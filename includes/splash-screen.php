...
class n360_SplashScreen {
	...
	function n360_splash_page() {
		if ( isset( $this->config['enable_n360_ss'] ) && $this->config['enable_n360_ss'] == 1 ) {
			...
			setCookie( $this->cookie_name, "n360_splash_screen", $this->cookie_expiration, "/", $domain, ( ! empty( $_SERVER['HTTPS'] ) && $_SERVER['HTTPS'] !== 'off' ) );
            $_SESSION['n360_just_shown'] = true;
	...
	public function n360_check_run() {
        if ( isset($_SESSION['n360_just_shown']) ) {
            unset($_SESSION['n360_just_shown']);
            $this->run = false;
            return;
        }
        if ( isset(get_option('n360_config')['show_every_time']) && get_option('n360_config')['show_every_time'] == 1 ) {
            $this->run = true;
            return;
        }
